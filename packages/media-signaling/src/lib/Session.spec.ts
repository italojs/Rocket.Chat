...
describe('MediaSignalingSession', () => {
	...
	describe('getState', () => {
		it('reports no call while the server has not confirmed the one this session placed', async () => {
		...
		it('still reports no call after the server refuses the call request', async () => {
			...
			const onPreventedCall = jest.fn();
			session.on('preventedCall', onPreventedCall);
			...
			expect(onPreventedCall).toHaveBeenCalledTimes(1);
			expect(session.getState()).toBeNull();
		});

		it('exposes the unconfirmed call to a caller that asks for it', async () => {
			const { session } = makeSession();
			await registerSession(session);
